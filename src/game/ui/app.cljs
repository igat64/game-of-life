...
(defn start! [app-state]
  (let [id (js/setInterval #(tick! app-state)
                           (:speed @app-state))]
    (do (swap! app-state assoc :interval-id id)
        (swap! app-state assoc :running? true))))

(defn stop! [app-state]
  (do (js/clearInterval (:interval-id @app-state))
...
(defn change-speed! [app-state speed]
  (do (swap! app-state assoc :speed speed)
...
(defn change-size! [app-state size]
  (let [new-board (b/create size)]
    (do (swap! app-state assoc :board new-board :gen 0 :running? false)
        (stop! app-state))))

(defn clear-board! [app-state]
  (let [size (get-in @app-state [:board :size])]
    (do (swap! app-state assoc :board (b/create size) :gen 0 :running? false)
        (stop! app-state))))
